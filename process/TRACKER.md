...
**Ongoing collection (PT-78): a local OTel receiver, off by default.** `/setup-tracker` opts a project in by adding this `env` block to the user's own settings file (`$CLAUDE_CONFIG_DIR/settings.json`, else `~/.claude/settings.json`) — Claude Code >= 2.1.282 ignores all five of these keys in project or local settings:
...
**Lifecycle (PT-86): the receiver starts with the first session on the repo and stops with the last, once every live session on the repo started under this hook line** (see the upgrade note immediately below for the one-time transition window where that isn't yet true) **— so in steady state, one teammate's `/exit` never drops another's live telemetry.** Every `SessionStart` (main session or teammate — each is its own process, so each counts) registers its session id in `process/cairn/metrics/.sessions/` (one small file per id, holding a liveness-probe pid — `$PPID`, the hook shell's own parent, measured to be the Claude Code session process itself); every `SessionEnd` deregisters it. A background watchdog thread inside the receiver, not the short-lived hook-invoked CLI processes, owns the actual decision: when the registry drains from non-empty to empty it arms a grace window (`--grace-period-seconds`, default 10s — long enough for the OTel exporter's own final export to land), cancels it if a new session registers before the window elapses, and otherwise flushes, removes its pidfile, and exits. A liveness probe (`os.kill(pid, 0)`) reaps a session that crashed without ever sending its `SessionEnd` — on every watchdog tick, at each flush, and (independently, so the all-sessions-crashed case is bounded rather than pinned on some unrelated future session's `SessionEnd`) a slow periodic sweep (`--periodic-reap-seconds` — accepted on the CLI for back-compat, has no effect since every tick already reaps). §0 of the architect's gating ruling on PT-86 is why every one of these choices breaks toward not stopping: a false pin costs only an idle localhost process; a false reap silently costs a live session's remaining telemetry. `--stop` still works for manual use; `--status` reports the live-session count and each id's liveness — `alive`, `dead` (pid probe says dead, reap-eligible on the next tick), or `unknown` (pid never captured, never reaped). **Measured (2026-09-04, PT-86): a teammate's `SessionEnd` hook fires on `shutdown_request` and its hook input carries the session id, so a teammate's exit deregisters it directly; the liveness probe is the backstop for crashed sessions only. Teammate registration on `SessionStart` was confirmed in the same run.**
...
1. **Telemetry gate (H1).** `ensure_running` registers the calling session first, then requires `CLAUDE_CODE_ENABLE_TELEMETRY` truthy in its own environment (`"1"`/`"true"`/`"yes"`/`"on"`, case-insensitive — the same string shape this project's other boolean env flags already use) before spawning; if untruthy, it spawns nothing and the hook exits 0, printing one stderr line naming that Claude Code >= 2.1.282 reads this var from `~/.claude/settings.json`, not project settings. **Correction (POLY-10, measured 2026-09-24 via `ps eww` on the live receiver): the `env` block reaches hook-spawned subprocesses for `CLAUDE_CODE_ENABLE_TELEMETRY` only — the `OTEL_*` exporter variables do NOT.** The harness applies those to its own OTel SDK exclusively; a hook-spawned process (the receiver itself, `--ensure-running`, `--session-ended`) never inherits them, same as a tool-subshell Bash call. So this block controls the receiver's own start/stop gate, but not — as previously written here — "both the exporter and the receiver": the exporter's *destination* is never influenced by anything a hook-spawned process can see. See H3 below for the consequence.
...
3. **Port/endpoint agreement (H3).** Before spawning, `ensure_running` resolves the expected endpoint via `_exporter_endpoint(environ, user_settings_path)`: `environ["OTEL_EXPORTER_OTLP_ENDPOINT"]` first, else `env.OTEL_EXPORTER_OTLP_ENDPOINT` inside the user's own settings file (`$CLAUDE_CONFIG_DIR/settings.json` else `~/.claude/settings.json`), else `_effective_endpoint_port`'s OTLP default (4318) — project settings are never consulted. If the resolved port disagrees with `otel_port`, it refuses to start, naming both and the resolution source (`env`/`user-settings`/`default`) — a mismatch doesn't lose telemetry, it silently delivers it to whatever else is listening on the wrong port, exactly PT-79's real incident. `--status` reports a matching `exporter-endpoint: <url> (source: ...)` line from the same resolver.

**The watchdog flushes on its own tick, every `--flush-interval`, independent of exports** — not only when `_on_export`'s own elapsed check fires, so an otherwise-quiet interval still advances `--out-file`'s `last-flush` marker. **A datapoint whose `session.id` has no transcript anywhere `backfill_tokens._transcript_path_for` looks (this repo, or a worktree sibling of it) is dropped before folding** — the exporter's destination is a single user-global endpoint, so every project on the machine with telemetry on shares the same port; a flush that dropped any prints one count-only stderr line (`dropped <n> datapoint(s) from <k> session(s) with no transcript in this repo`), never by session id.
...
**Every CLI invocation anchors on the main checkout, regardless of which worktree's own script copy is running.** `main()` resolves `repo_root` via `worktree_root.main_checkout_root(...)` (new module; `_resolve_worktree_main_checkout` moved here verbatim from `run_tests.py`, which re-exports the same name) once; `--ensure-running`/`--session-ended`/`--status`/`--flush-now`/`--stop` all derive their pidfile, sessions dir, out-file, and logfile from it. A failed `--flush-now`/`--stop` names the real, main-checkout pidfile it resolved (`error: no running receiver (<label>): pidfile <abs path> absent or stale`).
