...
**Ongoing collection (PT-78): a local OTel receiver, off by default.** `/setup-tracker` opts a project in by adding this `env` block. **Correction (POLY-25/POLY-49 ruling §5, M7): as of Claude Code >= 2.1.282, this block belongs in the USER's own `$CLAUDE_CONFIG_DIR/settings.json` (else `~/.claude/settings.json`), never the project's `.claude/settings.json`** — that release stopped applying ANY of these five keys from project or local settings at all (each session prints an ignored-vars notice when they're found there instead; measured, `aa3a21f`, this project's own `.claude/settings.json` no longer carries any of them). This is a broader regression than just the exporter: `CLAUDE_CODE_ENABLE_TELEMETRY` also used to reach a hook-spawned process's own env from PROJECT settings (POLY-10, pre-2.1.282) — it does so from USER settings under 2.1.282 instead (M6), which is what the receiver's own `--ensure-running` telemetry gate (H1) now depends on:
...
**Lifecycle (PT-86): the receiver starts with the first session on the repo and stops with the last, once every live session on the repo started under this hook line** (see the upgrade note immediately below for the one-time transition window where that isn't yet true) **— so in steady state, one teammate's `/exit` never drops another's live telemetry.** Every `SessionStart` (main session or teammate — each is its own process, so each counts) registers its session id in `process/cairn/metrics/.sessions/` (one small file per id, holding a liveness-probe pid — `$PPID`, the hook shell's own parent, measured to be the Claude Code session process itself); every `SessionEnd` deregisters it. A background watchdog thread inside the receiver, not the short-lived hook-invoked CLI processes, owns the actual decision: when the registry drains from non-empty to empty it arms a grace window (`--grace-period-seconds`, default 10s — long enough for the OTel exporter's own final export to land), cancels it if a new session registers before the window elapses, and otherwise flushes, removes its pidfile, and exits. A liveness probe reaps a session that crashed without ever sending its `SessionEnd` — on every watchdog tick now (**POLY-49 ruling §4, superseding the two-signal design below**: pid liveness alone, `os.kill(pid, 0)`, decides; the session's own transcript going quiet for 30+ minutes was a second signal PT-86 originally required, withdrawn because it could delay a dead entry's removal past AC7's "within one watchdog beat of its process exiting" bound), plus at each flush and (independently, so the all-sessions-crashed case is bounded rather than pinned on some unrelated future session's `SessionEnd`) a slow periodic sweep (`--periodic-reap-seconds` — accepted on the CLI for back-compat, but a no-op now that every tick already reaps). §0 of the architect's gating ruling on PT-86 is why every one of these choices breaks toward not stopping: a false pin costs only an idle localhost process; a false reap silently costs a live session's remaining telemetry. `--stop` still works for manual use; `--status` reports the live-session count and each id's liveness — `alive`, `dead` (pid probe says dead, reap-eligible on the next tick), or `unknown` (pid never captured, never reaped) — **three states, not four: POLY-49 ruling §4 withdrew the `dead-pending` label along with the two-signal predicate it diagnosed.** **Measured (2026-09-04, PT-86): a teammate's `SessionEnd` hook fires on `shutdown_request` and its hook input carries the session id, so a teammate's exit deregisters it directly; the liveness probe is the backstop for crashed sessions only. Teammate registration on `SessionStart` was confirmed in the same run.**

**Upgrading an existing checkout to PT-86 (one-time, not steady state — this is what the lifecycle paragraph above's opening qualifier refers to):** a receiver already running before this change has no handler for the nudge signal PT-86 introduces — run `python3 scripts/cairn/otel_receiver.py --stop` once after pulling, before the next session ends, so the old daemon's existing `SIGTERM` handler flushes cleanly instead of being torn down mid-flush by something it doesn't understand. Separately: any session that was already running when the upgrade landed started before the `SessionEnd` hook line existed, so it is not in the new registry — if a session that starts *after* the upgrade then starts and ends, the registry can go 1 → 0 and the receiver can stop while those older sessions are still live, and their exports go nowhere until their own sessions eventually turn over. That window can also close earlier than a session's own exit: `SessionStart` re-fires on `resume`, `clear` and `compact`, reading whatever hook lines are on disk at that moment — so a session that began before the upgrade registers itself the first time it compacts or resumes, without ever restarting. The catch is that its *end* then drains the registry, stopping the receiver while any sessions that have not yet compacted are still live and still unregistered. If `--status` shows the receiver stopped while sessions are still live, restart it bare (`python3 scripts/cairn/otel_receiver.py --ensure-running`, no session id) — a registry that has never been non-empty never arms the shutdown timer, so it stays up until the next natural session cycle picks it back up correctly.
...
1. **Telemetry gate (H1).** `ensure_running` requires `CLAUDE_CODE_ENABLE_TELEMETRY` truthy in its own environment (`"1"`/`"true"`/`"yes"`/`"on"`, case-insensitive — the same string shape this project's other boolean env flags already use); otherwise it returns `False`, spawns nothing, and the hook exits 0 quietly, printing one stderr line naming that Claude Code >= 2.1.282 reads this from `~/.claude/settings.json`, not project settings. **POLY-49 ruling §4: `register_session` now runs BEFORE this gate**, right after confirming a cairn tracker exists — a real-world "session never appeared" incident (M8) traced to H1 declining before registration ever ran (2.1.282 stripped `CLAUDE_CODE_ENABLE_TELEMETRY` from a hook-spawned process's env for a time); registration is a fast local file write that must succeed independent of whether a receiver ends up spawning. **Correction (POLY-10, measured 2026-09-24 via `ps eww` on the live receiver): the `env` block reaches hook-spawned subprocesses for `CLAUDE_CODE_ENABLE_TELEMETRY` only — the `OTEL_*` exporter variables do NOT.** The harness applies those to its own OTel SDK exclusively; a hook-spawned process (the receiver itself, `--ensure-running`, `--session-ended`) never inherits them, same as a tool-subshell Bash call. So this block controls the receiver's own start/stop gate, but not — as previously written here — "both the exporter and the receiver": the exporter's *destination* is never influenced by anything a hook-spawned process can see. See H3 below for the consequence.
2. **Bind verification (H2).** `ensure_running` doesn't just check the child is alive — it polls the port and distinguishes two failure shapes: a **different** process already holding `otel_port` (refuses to spawn a doomed second instance; very plausibly another project sharing the same default port) from a spawn that never actually came up (the child may have exited immediately — see the log). Either way it names the port and `otel_port`, prints to stderr, and returns `False`. The `SessionStart` hook line still always exits 0 (telemetry must never fail a session) but no longer redirects stderr to `/dev/null`, so these messages are visible instead of only ever reaching the gitignored log. `python3 scripts/cairn/otel_receiver.py --status` reports whether a receiver is running, its port, and its `--out-file` (exit 0 running / 1 not — see POLY-10 below for a third, `2`, exit code) — an operator-facing health check, distinct from `--ensure-running`.
3. **Port/endpoint agreement (H3).** **Fixed (POLY-25/POLY-49 ruling §5, resolving the follow-up this bullet used to end on):** before spawning, `ensure_running` no longer trusts its own inherited `OTEL_EXPORTER_OTLP_ENDPOINT` at all — POLY-10 measured that a hook-spawned process never sees it there in the real hook path, so the comparison used to always fall through to the OTLP default (4318) without ever seeing a real mismatch. `_exporter_endpoint(environ, user_settings_path)` now resolves the expected endpoint itself: `environ["OTEL_EXPORTER_OTLP_ENDPOINT"]` first (still wins on the rare chance it's actually set), else `env.OTEL_EXPORTER_OTLP_ENDPOINT` inside the USER's own settings file (`$CLAUDE_CONFIG_DIR/settings.json` else `~/.claude/settings.json` — the same file H1 now depends on, per M6/M7 above), else `(None, "default")`. `_effective_endpoint_port` still turns an unresolved endpoint into the real OTLP default (4318) for the comparison, unchanged. If the resolved port disagrees with `otel_port`, `ensure_running` still refuses to start, naming both (a mismatch doesn't lose telemetry — it silently delivers it to whatever else is listening on the wrong port, exactly PT-79's real incident) and citing the resolution source (`env`/`user-settings`/`default`); `--status` gained a matching `exporter-endpoint: <url> (source: ...)` line from the same resolver, so an operator never has to re-derive it by hand.

**Under-capture, root-caused and fixed twice over (POLY-49 ruling §1).** M1-M9 traced a 70-minute, 4-record flush to Claude Code 2.1.282 exporting nothing at all under the pre-fix settings (H1/H3 above), not to the aggregator dropping data. Two independent hardenings on top of the settings fix: (1) the watchdog now calls a flush from its own tick, every `--flush-interval` regardless of whether anything ever exported — the export-triggered check in `_on_export` alone could never fire if nothing exported, ever, even once the interval had long elapsed. (2) since the exporter's destination is now USER-global (one `~/.claude/settings.json`, M7), every project on the machine with telemetry on posts to the same `:4318` — a PT-79-class contamination risk `_handle_export_body` now guards against directly: a datapoint whose `session.id` has no transcript anywhere `backfill_tokens._transcript_path_for` looks (this repo, or a worktree sibling of it) is dropped before folding, logged once per flush as a count only (`dropped <n> datapoint(s) from <k> session(s) with no transcript in this repo`), never by session id.
...
**PT-82's worktree isolation does not disturb attribution, because the receiver never runs from a worktree.** Every teammate worktree sits at its feature branch's tip in a **detached** HEAD (git will not check the same branch out twice), which the branch-regex above would resolve to `main` if the receiver ran there. It doesn't: the receiver is started by the `SessionStart` hook, and hook commands resolve `${CLAUDE_PROJECT_DIR}` to the main checkout regardless of which worktree a teammate's own tool calls are `cwd`-scoped to — the main checkout stays on the feature branch for the session's whole life, so branch-first attribution keeps resolving correctly. This is a property of where the daemon starts, not of anything a worktree does; it would break only if the receiver were ever launched from inside a worktree instead.

**A separate CLI invocation FROM a worktree used to reach the wrong metrics dir entirely (carried finding, fixed POLY-49 ruling §3).** `--ensure-running`/`--session-ended`/`--status`/`--flush-now`/`--stop`, run directly against a teammate's own worktree copy of `otel_receiver.py` (not through the SessionStart/SessionEnd hooks, which only ever run in the main checkout), used to resolve every default path (pidfile, sessions dir, out-file, logfile) from `backfill_tokens._repo_root()` — that worktree's own root, never mounted as the `metrics` branch worktree at all — so a registration, a flush signal, or a status probe from a worktree silently missed the real running daemon. `scripts/cairn/worktree_root.py` (new; `_resolve_worktree_main_checkout` moved here verbatim from `run_tests.py`, which re-exports the same name) gives `main_checkout_root(p)`, which `main()` now applies once to anchor every one of those defaults at the MAIN checkout, regardless of which worktree's own script copy is actually running. A failed `--flush-now`/`--stop` now names the real, main-checkout pidfile it resolved (`error: no running receiver (<label>): pidfile <abs path> absent or stale`), not the worktree's own never-mounted one.
...
**Role resolves per `session.id`, through a transcript-header lookup, not from the OTel payload directly — and `agentSetting`, not `agentName`, is the primary field (PT-87).** Measured (amendment B, 25d7a42): `agent.name` is absent on real teammate-process exports, so the receiver reads `<transcripts-dir>/<session.id>.jsonl` (same slug derivation `backfill_tokens.py` uses) via the shared `backfill_tokens.resolve_role_from_header`. Precedence: `agentSetting`, present and non-empty, wins **verbatim** — no roster gate, no `-<digits>` strip, since it is the harness's own `subagent_type`, never a spawn nickname (a built-in type like `claude-code-guide` has no roster file and would otherwise lose to a spawn nickname, which is why the gate was dropped rather than kept); a **non-roster `agentSetting` is still kept, and still reported on the same unmapped-names paper trail** `agentName` always had. Absent `agentSetting`, falls back to the pre-existing `agentName` path unchanged (PT-77's roster-anchored `-<digits>` stripping). Neither field present resolves to `team-lead`; no transcript at all resolves to `subagent-unattributed` — a loud guard, never a silent fold into `team-lead` — and that miss is deliberately **not** cached, since a session's transcript appears on its first turn and a later lookup should retry rather than pin an early false negative. **This is a forward-looking fix, not a retroactive one:** a live `source: "otel"` line flushed before PT-87 keeps the role it was flushed with permanently — `session.id` is discarded at flush by design (the privacy allow-list), so the identity needed to re-resolve an already-committed `otel` line was never persisted, and no future backfill re-run can reach it. Only `source: "transcript-backfill"` lines are re-bucketed by a re-run; PT-0.12's "live and backfilled lines alike" is true from PT-87 forward, not for lines committed before it.

**Measured (architect's PT-87 ruling, 69 real transcripts): `agentSetting` and `agentName` never share a record.** `agentSetting` lives exclusively on `type: "agent-setting"` records (541 occurrences, zero on `assistant`); `agentName` appears on `user`/`attachment`/`assistant`/`system` records but never on an `agent-setting` one. Both consumers therefore scan **every record type** within the first 50 records — `assistant` records included — for the header fields — a separate, narrower question from the `type == "assistant"` filter usage accumulation still uses, which is unchanged. First `agentSetting` occurrence measured at line 1 (52 of 69 files) or line 2 (2 of 69) — a 25× margin over the 50-record window, which also doubles as `agentName`'s own fallback budget (one shared constant, not two). Both fields are measured **session-constant** (`agentSetting` varies within a file in 0 of 69, `agentName` in 0 of 53), so resolving once per session/file — cached for the receiver's lifetime, recomputed once per file for the backfill — is correct, not merely convenient. If a future Claude Code version ever emits `agent.name` directly on a teammate's own OTel datapoints, that value wins outright over the transcript lookup entirely (one condition to flip, unrelated to `agentSetting`/`agentName` precedence within the lookup itself).

**Verified end-to-end (2026-09-04, first telemetry-on session after the opt-in in #162): a real spawned teammate's live export resolves to a non-`team-lead` role through the running receiver.** Procedure — env block active at session start, receiver up from the SessionStart hook, one real teammate spawned for a single turn, `--flush-now` two minutes later — produced fresh `source: "otel"` lines for the teammate (`role` neither `team-lead` nor `subagent-unattributed`) and for the lead's own session; the run is recorded as a comment on PT-78. Two things the check established that are easy to get wrong when repeating it: the env block only takes effect for a session started *after* it is written, so the check cannot be run retroactively in an already-running session; and the `OTEL_*` variables are applied by the harness to its own exporter and are **not** visible in Bash subprocesses of either the lead or a teammate, or in hook-spawned processes (measured 2026-09-24, POLY-10) (only `CLAUDE_CODE_ENABLE_TELEMETRY` is) — `env | grep OTEL_` is not a liveness test; a fresh `source: "otel"` line after `--flush-now` is.

**Watchdog survival and health (POLY-10, gate-1 ruling `process/reviews/POLY-10/ruling.md` @ 3d83a20).** The watchdog thread treats an absent `.sessions/` registry dir as *unknown*, never *empty* — it holds (no reap, no grace-deadline change) for up to `--registry-absent-recreate-seconds` (default 60s; comfortably longer than `ensure_metrics_worktree.py`'s swap-aside window), then recreates the dir and its two startup markers rather than hold forever. Any OTHER watchdog exception (not the absent-dir case, not a caught `.closing`-open race) exits the whole receiver loudly — code 3, port freed, pidfile removed, a `watchdog: fatal` stderr line — rather than leave a silently-dead thread under a still-listening socket; the next `SessionStart`'s `--ensure-running` respawns it. `--status` gained a `watchdog: alive|stale|absent (last beat <iso>)` line (freshness judged by the heartbeat file's own mtime, written at most once per second, skipped while the registry dir is absent) and a `last-flush: <iso> (<n> lines)` line, plus a third exit code, **2**, for the one actionable forbidden state: running with a *stale* heartbeat (the dir is present but the watchdog has stopped ticking). An *absent* heartbeat — including the legitimate hold window above — does **not** trigger exit 2, deliberately: PT-86 §0's own asymmetry (a false stop costs a live session's telemetry; a false "everything's fine" costs nothing this ticket didn't already accept) applies the same way to `--status`'s own alarm.

**Teammate attribution now resolves through worktree-sibling transcript dirs as well as the main checkout's own.** Root cause (measured 2026-09-24): every team-agent session's cwd is `.claude/worktrees/<name>/`, so Claude Code files its transcript under the sibling slug `<main-slug>--claude-worktrees-<name>/`, never the main slug dir role resolution used to read exclusively — every teammate session landed as `subagent-unattributed`. `_transcript_path_for` now checks the direct `<transcripts_dir>/<id>.jsonl` path first (unchanged), then the first matching `<transcripts_dir.name>--claude-worktrees-*/<id>.jsonl` sibling (glob cost measured at 0.09ms/miss); used by role resolution and (POLY-49 ruling §1.2) the foreign-session export filter alike. **Superseded by POLY-49 ruling §4:** the dead-pid transcript-staleness probe and `--status`'s `dead-pending` label this sentence used to also credit are withdrawn — liveness reaps on pid alone now (see the Lifecycle paragraph above) — so this fix's teammate-liveness half is moot; its role-resolution and (now) foreign-session-filter halves remain live. `backfill_tokens.py`'s own (separate) transcript scan now shares this fix (POLY-26): `scan_transcripts` walks the main dir plus every `_worktree_sibling_dirs` match, so a backfill run attributes teammate lines by role the same way the receiver does. The remaining gap is issue attribution, not role: most teammate records carry `gitBranch: worktree-<name>`, which `_bucket_for_branch` maps to `main` (then to the active milestone), not the issue the teammate was actually working — follow-up filed: POLY-45.

**Main-branch usage is attributed to the milestone active at the record's own timestamp, never to an undifferentiated `main` bucket (PT-84).** A milestone's window is `[creation commit of its file, creation commit of the next milestone's file)` — half-open, so a record landing exactly on a boundary belongs to exactly one window (the later one) by construction. Measured against all 14 real milestones (architect's ruling, 81a9d96): issue-derived dates fail on coverage (`PT-0.3` has zero issues) and on collision (`PT-0.4`/`PT-0.5`/`PT-0.5.1` all share `created: 2026-08-20`); archive-move timestamps are retroactive bulk-commit bookkeeping (five milestones archived in one commit, two days after a sixth was archived separately) and overlap six ways — creation-commit timestamps are the only source that is unique, non-overlapping, and defined for every milestone, including one with no issues at all. The window's own start/id lookup is deliberately split by source of truth (`cairn.milestone_windows`): the milestone **id** comes from the file's *current* HEAD frontmatter (never a historical git blob — the first six milestone files were committed with an unprefixed, quoted id in their own creation-commit content as well as an unprefixed filename), while the **timestamp** comes from a per-file `git log --follow --diff-filter=A`, oldest entry — `--follow` is what walks a renamed or archived file back to its true origin commit. Ordering (both the on-disk `token-usage.jsonl` sort and the `/api/tokens` payload) ranks milestone buckets **after** all issue buckets and **before** `main`, by the milestone's own creation timestamp — never by string-comparing the id (`PT-0.10` sorts before `PT-0.5` lexicographically). A matching record's `issue` value is `milestone:<id>` verbatim from frontmatter, in the existing `issue` field — no new schema key.

**A collision (two milestone files resolving to the same, or an inverted, creation timestamp) degrades rather than aborts.** `git log --follow`'s own content-similarity rename detection can, in principle, mis-pair two near-identical milestone files (never observed against this repo's real milestones, whose `name` and definition-of-done prose differ substantially — reproduced only in a synthetic fixture built to differ *solely* in id/name). `milestone_windows` detects this (a duplicate id, or two ids sharing one timestamp after sorting) and, by default, **drops the colliding windows, warns loudly by name on stderr, and lets any record that would have matched a dropped window fall back to `main`** — under-attributed, never mis-attributed, never a stopped receiver. The alternative (raising) was the original ruling and was corrected specifically because it runs inside the receiver's own flush path: a milestone-file naming coincidence must never be able to abort telemetry collection outright (the same asymmetry PT-86 §0 already established — a loud, degraded result beats a silent stop). `strict=True` restores the raise as an isolated test seam for the detector itself, never used by a real collector.

**General rule, established twice now: an attribution-logic change is retroactive for `source: "transcript-backfill"` lines and prospective-only for `source: "otel"` lines.** A backfill re-run re-derives every `transcript-backfill` line from the raw transcripts under the new rule, so historical data is corrected. A live `otel` line, once flushed, keeps whatever attribution it was given at flush time permanently — the receiver only appends, and the identity/context needed to re-derive an already-committed line (a role's `session.id`, a record's own original timestamp-to-milestone mapping) is either discarded at flush by design (PT-87, the privacy allow-list) or simply never re-visited by anything downstream. PT-87 hit this first: three spawn-named `otel` lines, flushed before that ruling landed, can never be corrected. PT-84 hits it again: the 2026-09-04 AC 2 re-run left **6** pre-existing `otel` `main` lines dated on/after the first milestone's creation — each is *repairable in principle* (single-day windows, wholly inside one milestone's span, no straddling) but was deliberately **left as `main`, not repaired**, per the architect's call: six lines don't justify a one-off migration whose method only generalizes to the single-day case, and a repair invites reuse where it isn't sound. These are repairable but not repaired — not lines that will resolve on their own, since restarting a receiver changes what *future* lines get flushed as; it does not, and cannot, touch a line already on disk.

**Adopter remedy, same shape as PT-86's — and precisely what it does not do.** After pulling PT-84, restart any already-running receiver once (`python3 scripts/cairn/otel_receiver.py --stop`, then let the next session's `--ensure-running` bring it back) so the daemon picks up milestone-aware attribution for lines it flushes from that point forward — a receiver process already running in memory does not hot-reload the new `resolve_issue`/`milestone_windows` logic just because the code on disk changed underneath it. **This prevents further mis-bucketed lines; it does not fix existing ones.**

**Stopping the receiver before re-running the backfill, then restarting it bare (no session id) afterward, remains the recommended practice — but is no longer the only protection against loss (PT-89).** A backfill re-run moves the data file's `generated` stamp forward, past whatever the daemon has already buffered. `flush()` classifies each `(series, start_time)` group against that stamp individually: a group entirely before it is dropped (the backfill already counted those tokens), a group entirely after it is flushed normally, and a group whose own datapoints straddle the stamp is dropped whole and counted separately in the flush's log line — its aggregate value cannot be split, and keeping it would inflate a bucket with tokens the backfill already counted. The receiver keeps flushing in every case; a regen while it is running no longer breaks every subsequent flush for the life of the process the way it once did (measured during PT-84's own AC 2 run, architect's ruling 6304afd, and fixed by PT-89: the earlier guard's `raise` sat before the pending-state reset, so one refused flush poisoned every flush after it, silently, until the daemon exited). Stopping the receiver first still avoids the straddling case entirely — real, bounded data is genuinely lost there, not merely deferred — which is why it stays the recommended practice even though it is no longer the last line of defense.

**`/api/tokens` carries an explicit `kind: "issue" | "milestone" | "main"` per bucket, computed server-side** (`_token_bucket_kind`, from the raw `issue` string) **— the chart must never infer a bar's kind by string-sniffing the `milestone:` prefix itself**, since that couples the client to a storage-form convention the server owns and can change. The payload also carries `milestone_caption`, a one-clause, server-composed explanation of what milestone bars are — `null` when the payload has no milestone bucket, appended verbatim (never recomposed) by the chart's own caption when present. On the chart, milestone bars get the same treatment `main` already had: excluded from the top-N ranking cut applied to real issues, never re-ranked by value (they stay in the payload's own creation-time order), and rendered with `main`'s existing muted opacity — but still individually labelled by milestone id, and still their own bars, not folded into `main`.
...
**This is one flat snapshot, not a dated history.** `retrieved` records when the numbers were last checked; there is no `effective_from`/`effective_to` range. **Consequence: editing a rate here silently re-prices every past bucket that used that model, backfilled and live alike** — the dashboard has no way to know a rate changed on a particular date and will price August's tokens at today's number. If a rate ever actually changes (a re-verification at the same value is not a change), the fix is to promote each model's value to a list of `{effective_from, rates}` entries and have the one consumer (`build_tokens_payload` in `cairn.py`) pick by the contributing line's own `window_end` — a schema change, not a data edit. Until that's built, only edit this file when a rate is *unchanged but re-verified*, or accept that you are re-pricing history.

**An unpriced model never renders as free.** A model absent from `models` gets `cost_usd: null` on every bucket it contributes to (propagating up: a bucket with even one unpriced contribution has a `null`, not partial, total) — never `0`. Its token counts still render normally. `/api/tokens`' `prices.unpriced_models` names every such model so the dashboard's caption can say so, rather than silently under-counting the total. **The same posture holds for `cairn close`** (POLY-15 AC2, gate-1 ruling `process/reviews/POLY-48/ruling.md` §1(a)): an unpriced model in the close window writes `actual.cost_usd: null` and no `ratio` — never a partial total — prints a stderr warning naming the model(s), and the calibration record's `actual.cost_usd` is null too. A partial cost is rejected as a ruling: once written to frontmatter or `calibration.jsonl` it is a number indistinguishable from a full one, the stderr warning does not travel with it, and it would enter the estimate median. The repair is cheap — add the model's rate to `prices.json` and re-close `--at` the same sha.

**`GET /api/flow` is a throughput view, not a cumulative one (PT-85, replacing PT-61).** PT-61's original payload plotted a stacked area of live status counts per day — a `done` count that only ever grows reads as "how much work happened" when it isn't; the top of the stack could say "79 done" on a day four issues actually closed. PT-85 retires that shape entirely (no toggle — architect's ruling: a toggle preserves the exact artifact that produced the misreading, for the reader most likely to misread it again) and replaces it with **opened**, **closed**, **cancelled**, and **wip** per period, overall and per milestone.

**Day buckets are UTC, unchanged, not touched by this ticket.** `_parse_flow_events`'s `day` was already `commit_dt.astimezone(datetime.timezone.utc).date().isoformat()` before PT-85 — an explicit UTC conversion, confirmed by execution (a 23:30 PDT commit buckets to the *next* UTC day) after an earlier draft of this ruling incorrectly asserted it was local-offset (inferred from `%cI` carrying a local offset in the git format string, without reading the line that actually computes `day`). Half-open `[start, next)` boundaries; week buckets group those UTC days from **ISO Monday**, computed client-side (see below) — the server only ever emits day granularity.

**Derived from the SAME git walk, never a second git pass over issues.** `_compute_flow_payload` already maintained `live: {stem -> status}` while reconstructing PT-61's snapshots; throughput reuses that exact walk. `opened` is a stem's first-ever appearance (a `seen_stems` set, never cleared on delete); `closed`/`cancelled` key on a status **transition** (previous status ≠ target, target = `done`/`cancelled`), never a raw status read — re-adding an already-`done` stem must not re-count it. **The archive-move trap:** the walk keys on `Path(path).stem`, so `issues/PT-86.md` and `archive/issues/PT-86.md` are the same stem — an archive move surfaces as a delete-plus-add of that stem, and a naive "opened = appears after being absent" rule would count a real bulk-archive commit (PT-84 measured one touching seven files at once) as seven new opens on a day nothing opened. `seen_stems` is the fix: a deletion removes the stem from the live/WIP snapshot but never from `seen_stems`.

**WIP (`in-progress` + `in-review`) is point-in-time at period end, never an activity count.** Not `todo`/`backlog` — those are queue depth, not work in flight. A week view **sums** the period's `opened`/`closed`/`cancelled` deltas but takes the **last** day's `wip` in that week, never a sum or a mean — summing a point-in-time count is exactly the category error PT-85 exists to remove, restated in a new shape if the client got it wrong.

**`by_milestone` is dense from a milestone's first appearance onward — every day, with or without events.** The payload mixes two different gap semantics: `opened`/`closed`/`cancelled` are deltas (a missing day means zero) while `wip` is point-in-time (a missing day means *unchanged*). A sparse dict would force every client to know which rule applies to which key — the same inconsistent-measure defect this ticket exists to remove, reproduced in the schema. Every milestone already seen gets an entry on every subsequent point: zeros for the deltas, the carried value for `wip`.

**Milestone identity is canonical; assignment stays historical.** A record's milestone comes from the milestone recorded in **that event's own blob** (a historical reconstruction — using an issue's current milestone would retroactively move past work between milestones), but the *id* that assignment is filed under is canonicalised: a bare, pre-PT-28 id (`0.6.1`) and its current prefixed form (`PT-0.6.1`) are the same milestone, not two — PT-28's id-prefixing migration split several real issues' historical `milestone:` values this way, and filing them separately would leave every affected milestone's history incomplete under both its old and new id, which is worse than the "erase history" outcome canonicalisation is meant to avoid. Canonicalisation: apply the tracker's configured prefix if the raw value doesn't already match a known milestone record, and accept the result only if *that* matches a known record (current or archived) — checked against real data, never invented (the same discipline that let `agentSetting` win outright in PT-87). An id that still doesn't resolve survives as its own entry with `name: null` — a genuinely unknown id, not a mis-read known one.

**`milestones` lists every id the walk saw, `name`/`status` from CURRENT records.** Archived and historical milestones stay selectable — dropping an id with no current file would erase real, selectable history from the scope control, so it appears with `name: null, status: null` instead. `default_milestone` is whichever milestone's latest **transition** day (a day with a non-zero `opened`/`closed`/`cancelled` — never standing `wip` alone, or an abandoned milestone with stale in-progress issues would stay the default forever) is most recent; ties break toward the milestone whose file was created **later** in real history (via `milestone_windows`, never a numeric or lexicographic id compare — the real shape this matters for is a lower-numbered hotfix milestone opened after a higher-numbered one is already live).
...
`completed` and `active` fail the lint. The break is lint-only: an unmigrated repo's
...
- **Glob semantics.** Patterns are repo-relative, `/`-separated. `**` as a WHOLE segment matches zero or more segments (`src/auth/**` matches `src/auth/a.py` and `src/auth/x/y.py`, not `src/authz/a.py`). `*` matches within one segment, never crossing `/`; `?` matches one non-`/` char. No `[...]` classes, no special-casing of dotfiles. A pattern with no wildcards matches exactly one path. A hand-typed `paths:` entry that **starts with `*`** (`*.py`, `**/**`) must be **quoted** (`"*.py"`, `"**/**"`) — unquoted, cairn's YAML-subset parser reads a leading `*` as an alias, not a glob, and rejects it before `guard-push`'s own glob validation ever runs (POLY-9). `cairn new --paths` / `cairn set paths=` already quote such entries when writing, so this only bites a hand edit.
...
### Loop guardrails (PT-94)

The tracker carries the four-gate loop's mechanical enforcement (rules and rationale: `process/WORKFLOW.md` → Four gates and the loop caps; measurements: `process/reviews/PT-94/`).

- **`cairn gate --head <verified-sha>`** prints `git diff --stat <sha>..HEAD` and `PASS`/`FAIL`: PASS when every changed path is under `process/`, `docs/`, `temp/`, or ends in `.md`. Exit code follows the verdict. This is the head-match check at the finish gate; it replaces a re-verify round.
- **`cairn loop-stats <ID> [--base main] [--since ISO] [--until ISO] [--transcripts-dir DIR] [--steps DIR] [--json]`** prints the per-loop scorecard for the current feature branch: commits and chore commits (`base..HEAD`), comments and ruling sections in the issue file, KB added to it, tests added, full-suite runs and messages to the lead read from the harness transcripts inside the window (first branch commit → now, by default), idle notifications and how many duplicate the report they followed, and `cost_usd` from the tokens payload. Soft caps: 40 messages, 3 ruling sections, 15 commits, 4 full-suite runs; `suite_seconds_added` prints `(unmeasured — PT-93)` until the tiered runner records runs. `--steps DIR` writes one `steps-<role>.md` per transcript with every tool call classified in order (the PT-94 audit's format) plus `lead-inbound.md`. Transcripts decay with the harness's retention window; the written tables are the durable record.
- **`cairn comment`** refuses to append while the file carries another author's uncommitted comment (`--allow-foreign` overrides, for a deliberate sweep). **`cairn guard-commit`** is the body of `.githooks/pre-commit`: a staged tracker file whose added comments come from more than one author fails the commit, naming both. The SessionStart hook sets `core.hooksPath .githooks` idempotently, so the guard is on in every checkout that has opened a session.
- **`cairn check`** additionally **warns** (exit code unchanged) on an issue comment over 40 lines, an issue file over 24 KB, and a `TRACKER.md`/`WORKFLOW.md` paragraph over 8 sentences; it **errors** on an instruction-shaped phrase in those two docs outside quotes, code spans, and fences (`say why`, `state both`, `not just`, `worth noting`, `worth stating`, `flag it/this/that`, `be sure to`, `make sure to`, `don't forget`).
- **`process/reviews/<ID>/`** is the home for review logs, constructions, and audit tables an issue file must not carry (B6, D14) — tracked, referenced from the issue by path.
