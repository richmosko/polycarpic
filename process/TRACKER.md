...
**Lifecycle (PT-86): the receiver starts with the first session on the repo and stops with the last, once every live session on the repo started under this hook line** (see the upgrade note immediately below for the one-time transition window where that isn't yet true) **— so in steady state, one teammate's `/exit` never drops another's live telemetry.** Every `SessionStart` (main session or teammate — each is its own process, so each counts) registers its session id in `process/cairn/metrics/.sessions/` (one small file per id, holding a liveness-probe pid — `$PPID`, the hook shell's own parent, measured to be the Claude Code session process itself); every `SessionEnd` deregisters it. A background watchdog thread inside the receiver, not the short-lived hook-invoked CLI processes, owns the actual decision: when the registry drains from non-empty to empty it arms a grace window (`--grace-period-seconds`, default 10s — long enough for the OTel exporter's own final export to land), cancels it if a new session registers before the window elapses, and otherwise flushes, removes its pidfile, and exits. A liveness probe (`os.kill(pid, 0)` **and** the session's own transcript file going quiet for 30+ minutes — both signals, so a single mis-detected pid can never silently cut off a live session's telemetry) reaps a session that crashed without ever sending its `SessionEnd` — on the next `end` event, at each flush, and (independently of both, so the all-sessions-crashed case is bounded rather than pinned on some unrelated future session's `SessionEnd`) a slow periodic sweep (`--periodic-reap-seconds`, default 5 minutes; worst case: one periodic-sweep interval plus the 30-minute transcript-staleness floor). §0 of the architect's gating ruling on PT-86 is why every one of these choices breaks toward not stopping: a false pin costs only an idle localhost process; a false reap silently costs a live session's remaining telemetry. `--stop` still works for manual use; `--status` reports the live-session count and each id's liveness — `alive`, `dead` (both signals agree, reap-eligible now), `dead-pending` (pid gone, transcript still fresh — protected from reaping for up to 30 minutes; the diagnostic case for "why won't the receiver stop"), or `unknown` (pid never captured, never reaped). **Measured (2026-09-04, PT-86): a teammate's `SessionEnd` hook fires on `shutdown_request` and its hook input carries the session id, so a teammate's exit deregisters it directly; the liveness probe is the backstop for crashed sessions only. Teammate registration on `SessionStart` was confirmed in the same run.**
...
3. **Port/endpoint agreement (H3).** Before spawning, `ensure_running` compares `otel_port` against the port named by its own **inherited** `OTEL_EXPORTER_OTLP_ENDPOINT` — not by parsing `settings.json`, since the telemetry env may legitimately live in `settings.local.json` or a shell profile instead. If the variable is set and the ports disagree, it refuses to start, naming both (a mismatch doesn't lose telemetry — it silently delivers it to whatever else is listening on the wrong port, exactly PT-79's real incident). **Correction (POLY-10): the sentence that used to follow here — "if the variable is unset, H1 already declined before this check is ever reached" — is false as written.** H1 gates on `CLAUDE_CODE_ENABLE_TELEMETRY`, which *does* reach the hook-spawned `ensure_running` process; `OTEL_EXPORTER_OTLP_ENDPOINT` does not (per H1's correction above), so in the real hook path H1 passes while the endpoint variable is *always* unset — meaning this comparison always falls through to `_effective_endpoint_port`'s OTLP-default fallback (4318) rather than ever seeing a real mismatch, in every live-hook invocation to date. The check is not dead code (a manually-invoked `--ensure-running` with the variable exported by hand still exercises it, and did during PT-81's own construction-verified tests), but it has never once fired against a real hook-spawned mismatch. Follow-up filed: POLY-25 (read the expected endpoint from `settings.json`/a cairn config key instead of the environment).
