...
- **One writer per checkout — now enforced by isolation, not by convention.** Every teammate works in its own git worktree at the feature branch's tip (PT-82, superseding the earlier "shared checkout by default, worktrees only on escalation" framing): `feature/<id>-<slug>` on origin is the sole integration point for an issue, and a deliverable does not exist until it is a commit pushed there — the sender commits locally by pathspec, pulls `--rebase`, pushes fast-forward, and reports the full sha; the receiver verifies from the commit object (`git show <sha>:<path>` against `origin/<feature-branch>`), never from the sender's worktree. This is identity, not a brief — every agent definition in `.claude/agents/*.md` carries the shared worktree protocol block (`EnterWorktree` first, pull-rebase before a step, push fast-forward on completion, commit by pathspec, report a sha), deduped and machine-checked byte-identical across all ten files. One-writer-per-file remains the rule inside that; it is what keeps the pulls conflict-free.
- **Per-agent git identity, per worktree (POLY-1).** Each teammate sets its own `user.name`/`user.email` with `git config --worktree` right after `EnterWorktree` and before its first commit, so the feature branch's commit log attributes work by agent (the shared worktree-protocol block carries the exact commands). This is enabled by `extensions.worktreeConfig=true`, set repo-wide, idempotently, by the SessionStart hook. It changes attribution only: push credentials, the PR-opening GitHub account, and the model's `Co-Authored-By` trailer are untouched, and the lead's main checkout — which never enters a worktree — keeps the Principal's identity throughout. Decision record: `docs/project_kickoff.md` § 2.1.
...
### Loop discipline (learned in the PT-0.12 loops, 2026-09-04/05)
...
- **A green suite after a fixture change is a prompt, not a result.** Ask what stopped being exercised: a test whose subject can no longer vary is indistinguishable from a passing test. Three instances in one milestone. A reviewer pre-registers a mutation matrix — one named mutation per test that must turn it red — and runs it on a scratch copy with a control, including tests that were never seen red.
- **Constructions carry controls at both ends of the run**, and rule out the benign explanation of their own observation before a finding is real. A harness whose subject never started, a check that deregistered the only session, and grep patterns that could never match each produced a confident false finding against a teammate's work.
...
- **No direct pushes to `main`.** All changes — including doc-only updates on `phase/...` branches — merge via PR. **Two merge paths:**
  - **Human review** via GitHub's UI: review the diff, approve, click "Create a merge commit" (the only method the repo settings allow — squash would erase per-agent commit authorship from `main`).
  - **Team-lead merge** via `/merge-pr`: the user asks the lead to merge on their behalf after QA sign-off (for features) or lead's read of the diff (for doc updates). The skill verifies merge-readiness, flips the issue to done, updates state, and merges with a merge commit.
...
`process/cairn/metrics/` (hook-recorded test runs, backfilled/otel token usage) is a **nested git worktree of an orphan `metrics` branch, never merged into `main` or any feature/phase branch** (team-lead ruling, `process/cairn/issues/POLY-4.md` @ 5495132) — these files are append-only, cross-session, cross-teammate ledgers that would otherwise dirty every branch's working tree on every test run or receiver flush, with no meaningful diff to review. The branch carries its own `.gitattributes` (`*.jsonl merge=union`, so concurrent appends from different sessions merge rather than conflict) and its own `.gitignore` for the otel receiver's runtime scratch (pidfile, `.sessions/`, its log). Bootstrap is idempotent and automatic: `scripts/cairn/ensure_metrics_worktree.py` runs as a SessionStart hook step, mounting the worktree (fetching `origin/metrics`, or creating a fresh orphan if no repo has pushed one yet) the first time any checkout needs it — a pre-existing non-worktree directory there is swapped aside, restored (runtime scratch only, never a stale copy of a tracked file the checkout just supplied), and never silently deleted. `test_run_record.py` commits its own append inside that nested worktree after every run; pushing `metrics` to origin is `/finish-feature`'s job, not any hook's.
