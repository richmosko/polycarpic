...
- `legacy_archived_issue_paths(data_dir: Path) -> List[Path]` — PT-52: the legacy flat `data_dir/archive/*.md` layout, for exactly two callers — `check_repo`'s lint scan and `allocate_and_create_issue`'s allocation guard — which must never disagree about what counts as legacy. (POLY-6: `migrate_archive_issues`, its third caller, was retired along with the other one-shot migrations; this helper stays.)
