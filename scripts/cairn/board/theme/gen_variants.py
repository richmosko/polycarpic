...
def _resolve_values(dim: str, values: dict) -> dict:
    ...
    PT-120 gate-1 ruling (PT-120.md @fe2a929): Chart's flow/counter
    families no longer derive anything here -- PT-118's rotate+gamut-map
    machinery is deleted, not bypassed. They are now `var(--chart-N)`
    aliases declared ONCE in app.css, resolving to the selected variant's
    own `--chart-1..5` by cascade; `--chart-1..5` themselves are still
    emitted per variant exactly as before, untouched by this ruling."""
