...
### Chart-local ramp (PT-61 — issue flow over time) — retired 2026-09-05 (PT-85); PT-92's dedicated hues (2026-09-08) retired again 2026-09-09 (PT-120); now exact `--chart-N` aliases

**Retired 2026-09-05 (PT-85, architect ruling 586af1f).** The 6-token ordinal ramp this section originally documented (`--chart-flow-backlog`…`--chart-flow-cancelled`, one per `STATUS_ORDER` status) is **gone from `app.css` and every `variants.css` variant** — implementation-lead removed all six, in both modes, in the same commit as the ruling. Reason: PT-85 replaced the issue-flow chart's cumulative status-stack view with a throughput view (bars: opened/closed per period; line: WIP) — three **categorical** series that need to be told apart at a glance, not an **ordinal** progression through one hue family. `scripts/cairn/tests/test_chart_variant_ramps.py` and `ChartRampOrdinalValidationTests` were retired alongside. The history below (re-stepping rationale, the original 6 values, validator evidence) is kept as a design-history record, not as current token state.

**Current state (PT-120, 2026-09-09): `--chart-flow-{opened,closed,wip}` are `var()` aliases onto `--chart-1..5`, declared once in `app.css`, verbatim — no rotation, no gamut-mapping, no per-variant emission.**
...
Each Chart Color variant's `data-cairn-chart="<name>"` block already redefines `--chart-1..5`; the aliases resolve to whichever ramp is active by ordinary CSS cascade, so exactness holds **by construction, not by generation or assertion** — there is nothing to regenerate when a preset changes, and there's no separate literal to drift out of sync. Under `yellow` (see **First paint**, below) this resolves to:

| Token | Alias | Resolved (`yellow`) | Series |
|---|---|---|---|
| `--chart-flow-opened` | `var(--chart-1)` | `oklch(0.905 0.182 98.111)` | issues opened per period |
| `--chart-flow-closed` | `var(--chart-3)` | `oklch(0.681 0.162 75.834)` | issues closed per period |
| `--chart-flow-wip` | `var(--chart-5)` | `oklch(0.476 0.114 61.907)` | WIP line (point-in-time, end of period) |

One value per token, not light/dark — `--chart-1..5` is identical in `:root`/`.dark` for all 24 variants, confirmed (not assumed) in architect's PT-120 sweep.

**Why `opened=chart-1, closed=chart-3, wip=chart-5`:** the widest achievable spread available inside one 5-step ramp. Of the five 3-step subsets measured (architect, independently reproducing this choice), `(1,3,5)` is the only one clearing OKLab ΔE ≥0.06 with real margin in **every** one of the 24 variants — min **0.1195**; the alternatives measured worse in at least one variant: `(2,3,5)` 0.0596, `(2,4,5)` 0.0620, `(2,3,4)` 0.0512. `opened` takes `chart-1` — this ramp's step most likely to miss the 2:1 contrast floor (see **Observations**, below) — because it's the one series with a hue-independent identity backstop already built in (next paragraph).

**Line treatment (PT-92, ux-designer, still current — only the color values above changed, not this).** Full spec (stroke weight, dash/marker pattern per series, z-order, legend order, WIP-on-axis remedy) is recorded on the PT-92 issue file rather than duplicated here. Summary: `opened` solid stroke (bottom), `closed` dashed stroke on top of it (dash gaps expose `opened` at any coincidence), `wip` dotted stroke on top of both (also resolves WIP reading as the x-axis rule when wip=0); graduated per-series point markers (largest→smallest, opened→closed→wip) so an exact-coincidence point still shows a ring of each underlying color, not just the topmost. Legend fixed order: Opened, Closed, WIP. **This pattern layer is why the guard retirement below is survivable rather than a silent regression** — it was hue-independent when PT-92 built it and is exactly as effective now.

**Guard retirements (PT-120) — measured, then retired, never silently softened.** PT-92's ≥60° pairwise hue-separation guard does not fail occasionally under exact preset values; it fails **always, by construction**: a shadcn chart ramp's own hue drift across its 5 steps is a median 6.4° (max 45.4°), so the minimum pairwise hue gap among any three of those steps, across all 24 variants × 2 modes, is **0.2°** (violet) — **all 48 combinations fail**. `scripts/cairn/tests/test_flow_series_distinctness.py`'s pairwise/role-collision hue assertions are **retired**, the same precedented action as PT-85 retiring the ordinal-ramp test once its own assumption stopped holding. Replacing it: **min pairwise OKLab ΔE ≥ 0.06**, the property actually load-bearing now, measured (not inferred from the mapping choice) at **0.1195** across all 24 × 2. **No variant ever renders two flow series identically** — `--chart-1..5` is monotonic in lightness by construction (a non-monotonic step would already fail the base ramp's own ordinal validator), so `chart-1 ≠ chart-3 ≠ chart-5` holds with no exception to check for.

**Observations (not gates — the values are exact by mandate, so a floor miss is a fact about the vendored preset, not a bug to fix here):**
- **Contrast, 2:1 vs `--card`, two-model:** `chart-1` (`opened`) misses in **24/24** variants, light mode — on record since 2026-08-28, now directly visible for the first time; `chart-5` (`wip`) misses in **11/24**, dark mode; `chart-3` (`closed`) never misses.
- **Gamut:** **62 of 120** vendored `--chart-1..5` values (24 variants × 5 steps) sit outside sRGB — a P3-authored palette by construction, `yellow` included (worst overshoot 0.064 linear-light, median 0.0163). The browser gamut-maps on render, same as it silently did before anything consumed these tokens. PT-118's unclamped hard-fail gate is **retired** — it would refuse the vendored data itself, which outranks the gate.

**First paint: no flash, and `app.css` now holds zero flow/counter color literals.** `index.html`'s inline bootstrap script sets `data-cairn-chart` only when the stored preference differs from `yellow`, the default — under `yellow`, no attribute is ever set, and `:root`'s own `--chart-1..5` is the permanent render path the aliases resolve against. Nothing is duplicated, nothing can drift out of sync.

<details>
<summary>Design history: PT-92's dedicated 3-hue flow palette (2026-08-28 → retired 2026-09-09, PT-120)</summary>

**What it replaced (PT-85's placeholder hues):** 3 dedicated categorical tokens, `--chart-flow-opened` / `--chart-flow-closed` / `--chart-flow-wip`, fixed and identical across every Chart Color variant (deliberately not chart-hue-selectable, by the reasoning of the time). Values placed at 3 new OKLCH points independently constructed for this purpose (not sourced from the dataviz skill's `palette.md` swatches verbatim — every one of that reference palette's 8 named colors measured within 25° hue of some `--chart-role-*` hue in at least one mode):

| Token | Light OKLCH | Dark OKLCH | Series |
|---|---|---|---|
| `--chart-flow-opened` | `oklch(0.55 0.20 16.9)` | `oklch(0.58 0.17 16.9)` | issues opened per period |
| `--chart-flow-closed` | `oklch(0.62 0.18 135.2)` | `oklch(0.65 0.15 135.2)` | issues closed per period |
| `--chart-flow-wip` | `oklch(0.62 0.15 200.8)` | `oklch(0.60 0.13 200.8)` | WIP line — briefly re-authored to `0.100`/`0.097` chroma under PT-118's now-deleted gamut-mapper, moot since PT-120 |

**Distinctness thresholds** (architect's pre-registration, PT-85 issue comment 9336826): pairwise hue separation ≥60° between the three flow tokens; ≥25° hue separation from every `--chart-role-*` hue (role hues: 47.5°/76.8°/105.0°/165.5°/236.2°/252.0°/300.0°/346.3°). Guarded by `scripts/cairn/tests/test_flow_series_distinctness.py`, retired 2026-09-09 (PT-120, see **Guard retirements** above).

**PT-92 resolution (ux-designer, 2026-09-08).** PT-85's values were interim pending ratification; PT-92 also moved the chart from filled `Bars` + one `Spline` to three overlapping line marks (coincident values, measured live, fully occluded ≥2 of 3 series on ~18% of points otherwise). Disposition: `--chart-flow-opened`/`--chart-flow-wip` ratified as-is (both modes); `--chart-flow-closed` **light mode only** replaced `oklch(0.68 0.18 135.2)` → `oklch(0.62 0.18 135.2)` (C/H unchanged) to clear the WCAG 1.4.11 non-text floor outright (3.44:1, was a 2.72:1 WARN) rather than lean on the dash-pattern relief alone. A deuteranopia re-simulation (Machado/Oliveira/Fairchild 2009) confirmed no new CVD risk band was crossed.

**PT-118's rotation + gamut-mapping mechanism (2026-09-09, superseded same-day by PT-120's exact-alias approach):** briefly derived these hues per Chart Color variant via a global hue rotation (`Δh` from the variant's `chart-1` vs `yellow`'s) plus a mechanical unclamped-gamut mapper. Deleted, not bypassed, once Mosko's "the color code should be EXACT" steer landed — the rotation approach never put a variant's rendered color *at* its own selected hue (Pink rendered brown, Cyan rendered purple), which is what PT-120 exists to fix. The role-family reasoning from this period is the one piece that survived unchanged — see **Counter-type palette**, below.

</details>

<details>
<summary>Design history: the retired 6-token ordinal ramp (PT-61, 2026-08-28 → retired 2026-09-05)</summary>
...
Resolution (now retired): **6 new chart-local tokens**, one per `STATUS_ORDER` status (`backlog`/`todo`/`in-progress`/`in-review`/`done`/`cancelled` — the same taxonomy the flow chart's server payload was keyed by at the time), within the same golden hue family, snapped to clear every check the dataviz skill's ordinal validator runs (`validate_ordinal`: monotone lightness, ≥0.06 adjacent ΔL, ≤40° hue spread, ≥2:1 light-end contrast) — validated in **both modes**, against each mode's own real `--card` surface, not assumed identical because one mode happened to pass:
...
**Current state (PT-120, 2026-09-09): `--chart-counter-*` are `var()` aliases onto `--chart-1..5`, `chart-1` deliberately unused, declared once in `app.css` — same mechanism as flow, above.**
...
| Token | Counter | Alias | Resolved (`yellow`) |
|---|---|---|---|
| `--chart-counter-output` | `output` | `var(--chart-2)` | `oklch(0.795 0.184 86.047)` |
| `--chart-counter-cache-read` | `cache_read` | `var(--chart-3)` | `oklch(0.681 0.162 75.834)` |
| `--chart-counter-cache-write` | `cache_write` | `var(--chart-4)` | `oklch(0.555 0.135 66.442)` |
| `--chart-counter-input` | `input` | `var(--chart-5)` | `oklch(0.476 0.114 61.907)` |

**Why `chart-2..5`, skipping `chart-1`, preserving today's light=`output`/dark=`input` direction:** `chart-1` is the ramp's step already on record (2026-08-28) as missing the 2:1 contrast floor against white — counter has no pattern-based backup the way flow's dash/dot lines do (it's plain bar fills), and a spare step exists, so it's left unused here. Flow doesn't have that luxury (needs 3 of 5 for any spread at all) and takes `chart-1` anyway — see **Chart-local ramp**, above.

**Adjacency observation (not a gate — same posture as flow's guard retirements above):** contiguous `chart-5/4/3/2` leaves **4 of 24 variants with an adjacent-step OKLab ΔE below the 0.06 floor** this family was originally validated to: **violet 0.0512, indigo 0.0583, blue 0.0584, rose 0.0596**. The alternative mapping that avoids this (`chart-5/4/2/1`, skipping `chart-3` instead) has zero such variants but reinstates `chart-1`'s contrast miss on a bar fill with no pattern backup — worse trade. Recorded, not fixed: the values are exact by mandate.

**Role family (`--chart-role-1..8` + 3 guards) stays fixed — deliberately excluded from both this mapping and flow's.** Role hue is an identity mapping, not a stylistic one (PT-79 cross-checked each hue once against `--primary`/`--destructive`; PT-69 already ruled that a token's meaning must not silently change depending on which control the user is looking at — the Paused-badge/`--chart-2` collision this doc's **Badge/chip vocabulary** section records). Letting role hue track an unrelated Chart Color preference would reopen that defect, across two views (cost and tokens) of the same dashboard block. Unaffected by PT-120 — this reasoning predates it (PT-118) and nothing about exact-value aliasing touches role.

<details>
<summary>Design history: the dedicated fixed counter family (PT-79, 2026-09-03 → retired 2026-09-09, PT-118/PT-120)</summary>

**What this resolved originally:** implementation-lead found the *tokens* view of the PT-79 block illegible on adjacent steps of the golden `--chart-1..5` ramp, and worked around it by reusing four role hues (`--chart-role-1` for `input`, `-6` for `cache_write`, `-4` for `cache_read`, `-7` for `output`) — flagged back because that made `--chart-role-1` read as `team-lead` in the cost view and `input` in the tokens view of the same block.

**Ruling (ux-designer, 2026-09-03): reject the reuse, reject a wider-stepped `--chart-1..5`, ship a dedicated 4-step counter family.** Reusing role hues repeats the PT-69 collision class (a token's meaning shifting under a different control); re-widening the golden ramp doesn't fix the real problem, which is a single-hue-family shape being wrong for four *kinds* of thing, not a step-width problem. Shipped instead: a fixed family, its own hue (H 205°, muted cyan/teal, unused elsewhere), stepped in lightness like `--chart-flow-*`, at ~70% of gamut-edge chroma so the group read visibly quieter than the 8-hue role rainbow.
...
**Evidence.** All four cleared 2:1 on both real `--card` surfaces; adjacent-step OKLab separation 0.070–0.076 raw, 0.146–0.161 under simulated deuteranopia/protanopia (Machado/Oliveira/Fairchild 2009). Fixed across every Chart Color variant, identical `:root`/`.dark` — briefly made to rotate + gamut-map per variant under PT-118 (2026-09-09), then replaced same-day by PT-120's exact `chart-N` aliases once Mosko's "EXACT" steer landed, since a hand-tuned dedicated hue can't also be "the shadcn gradient the user picked."
...
**Architecture (ruled by architect, PT-69, 2026-08-29):** four dimensions. Three are CSS `data-cairn-*` attributes on `<html>` (Base Color → `data-cairn-base`, Theme → `data-cairn-theme`, Chart Color → `data-cairn-chart`); the fourth (Mode) reuses the existing `.dark` class. The three attribute dimensions **partition** the token set — no token belongs to more than one dimension, mechanically enforced by a test. Values are vendored, not fetched at runtime, from shadcn-svelte's own generator (browser-extracted, same method as the 2026-08-26 preset extraction) into `scripts/cairn/board/theme/variants.json`; a generator script emits three CSS copies — `scripts/cairn/board/variants.css`, `scripts/cairn/dashboard/src/variants.css`, and `docs/DESIGN/variants.css` (this system's copy). Persistence is one origin-global `localStorage` key, `cairn.theme`, shared by the dashboard and the board — a theme is a fact about the person looking, not the project being viewed, so it is deliberately not repo-scoped.
...
| **Chart Color** | `data-cairn-chart` | `--chart-1..5` (re-derived per variant). **Since PT-120 (2026-09-09), owns no other tokens directly** — `--chart-flow-{opened,closed,wip}` and `--chart-counter-{output,cache-read,cache-write,input}` are `var()` aliases onto specific `--chart-N` steps, declared once in `app.css` (see **Chart-local ramp**), so they track this dimension through the cascade rather than being separately emitted per variant. `--chart-role-1..8` deliberately excluded, stays fixed as an identity mapping | `yellow` | **Yellow** (default) + **the full reference set** — expanded 2026-08-29 (Mosko), the ≤5 cap is lifted; superseded my earlier curated 3 |
...
Chart Color's validation surface: every variant costs its own validator-passing derivation of the `--chart-1..5` sequential ramp, checked against the **extremes of `--card` across every Base Color × Mode combination**, not just the default Stone card: the ramp's light end is validated against the maximum-luminance `--card` in that set, its dark end against the minimum-luminance one, with both extremes computed from `variants.json` rather than hardcoded. Contrast against a surface is monotone in that surface's luminance on each side, so clearing the two extremes clears every base — which is what collapses the obligation from 15 validator runs to 2 **per variant** (now potentially many more variants than 3). **History (PT-85 → PT-118) and current state (PT-120, 2026-09-09):** flow and counter briefly had their own per-variant derivation (fixed, then rotated+gamut-mapped) and their own validation surface. Since PT-120 they're `var()` aliases onto `--chart-N` — **there is no separate flow/counter validation surface any more**; whatever the `--chart-1..5` sweep above already establishes for a variant is what flow/counter inherit, automatically, by the same cascade. What's left to check is a fixed, one-time set of **observations** (not gates) on the ramp itself: contrast misses on `chart-1`/`chart-5`/etc., 62/120 gamut misses, and the flow/counter ΔE-adjacency figures — see **Chart-local ramp** and **Counter-type palette** above. The role family (`--chart-role-1..8`) stays off this surface entirely, fixed, for the identity reasons given there — no 24×2 role-vs-`--primary` cross-dimension cost was ever incurred.
...
Not a rubber stamp: PT-61's finding was that a *published* shadcn palette failed a floor nobody had checked. **If a Base or Theme variant fails, drop it from the option set — do not re-derive it.** We are not in the business of repairing shadcn's published neutrals, and a re-derived "Zinc" that isn't shadcn's Zinc is worse than four base colors. **Chart Color was the deliberate exception through PT-92** (`--chart-flow-*` was this project's own token then, so a failing chart hue got re-stepped rather than dropped) — **reversed by PT-120 (2026-09-09):** flow/counter are now exact `--chart-N` aliases, Mosko's explicit steer, so a failing chart-N step is recorded as an observation (see **Chart-local ramp**) rather than re-stepped or dropped; `--chart-1..5` itself is still vendored shadcn data, same non-repair rule as Base/Theme. With the corrected pair list, **all 7 Theme options pass** — the dimension was never actually at risk; the `--sidebar-primary-foreground`/`--sidebar-primary` pair that would have gated it doesn't render anywhere.
...
| **Chart** | `Chart.Container config={ChartConfig}` wrapping `layerchart` primitives (`BarChart`/`Bars`, `Spline` overlay), `Chart.Tooltip` for hover | **Landed (PT-61), replaced 2026-09-05 (PT-85):** the cumulative status-stack area chart (`AreaChart`/`Area`) is retired entirely, not toggled — the dashboard's issue throughput chart now renders opened/closed as grouped bars (`BarChart`/`Bars`) plus a WIP line (`Spline`, sharing the bars' scale), milestone-scoped, day/week toggle, with a custom legend row (layerchart's own series-based legend only covers declared bar series, and adding the line series there would reserve an empty third bar band) — see **Chart-local ramp** above for the 3 color tokens it uses. Pulls in `layerchart` + `d3-scale`/`d3-shape` as real new dependencies (dynamically imported so they land in their own bundle chunk, not the main one) |
